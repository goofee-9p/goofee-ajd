...
TARGETS = ROOT / "index.html", ROOT / "이력서" / "이력서_구병기.html"
...
CLASS_TARGETS = {
    ...
    # 이 버전에서 새로 생긴 본문 클래스
    "bc-name", "bc-cat", "bc-d", "hi", "gb-x", "gb-note", "shot-c", "wall-cap",
    "tl-note", "am-tag", "end-note", "band-sub", "band-note", "hero-contact",
    "kr", "lead", "note", "quote", "cap", "mo",
}
# 안쪽 구조가 곧 의미인 요소 — 통째로 바꾸면 구조가 날아간다.
# .cont 는 li 로도 쓰여 TAGS 에 걸리므로 여기서 따로 막는다
CLASS_SKIP = {"cont"}
# 히어로 콘솔은 4장면이 겹쳐 돌아가 화면에서 고칠 수 없다 — HTML 에서 직접 고친다
SKIP_SUBTREE_IDS = {"console"}
SKIP_SUBTREE_CLASSES = {"console", "cs-scene", "mix", "orbit", "ax2"}
...
class Scanner(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if classes & CLASS_SKIP:
            is_target = False
        # 조상 중 하나라도 skip 서브트리면 그 안은 전부 제외
        in_skip = any(n.get("skip") for n in self.stack) or bool(classes & SKIP_SUBTREE_CLASSES)             or a.get("id") in SKIP_SUBTREE_IDS
        if in_skip:
            is_target = False
        node = {
            "tag": tag,
