...
"""로컬 편집 서버.
    ...
    → http://localhost:5173/이력서/이력서_구병기.html?edit
...
EDITABLE = {"index.html", "이력서_구병기.html"}
...
def rebuild_pdf(which: str) -> dict:
    ...
    if which != "resume":
        return {"ok": False, "error": "이 버전은 웹 전용입니다 — PDF 는 이력서만 만듭니다."}
    url = f"http://localhost:{PORT}/이력서/이력서_구병기.html"
    out = ROOT / "이력서" / "이력서_구병기.pdf"
...
if __name__ == "__main__":
    ...
    with Server(("127.0.0.1", PORT), Handler) as httpd:
        ...
        print(f"이력서    · http://localhost:{PORT}/이력서/이력서_구병기.html?edit")
        print("종료는 Ctrl+C")
        try:
            httpd.serve_forever()
