...
(function () {
  ...
  /* ── 편집을 방해하는 것들 끄기 ──
     ...
     숫자 카운트업·마퀴는 방금 고친 값을 되돌리거나 글자를 흘려보낸다 */
  document.querySelectorAll('.rv').forEach(function (el) { el.classList.add('on'); });

  /* 카운터는 애니메이션을 끄되 최종값은 채워 둔다 — 0 이 보이면 무엇을 고치는지 알 수 없다 */
  document.querySelectorAll('[data-count]').forEach(function (el) {
    var v = parseFloat(el.getAttribute('data-count'));
    if (!isNaN(v)) el.textContent = v.toLocaleString('ko-KR');
    el.removeAttribute('data-count');
  });

  /* 히어로 콘솔 루프·아카이브 마퀴·리본을 멈춘다 (GSAP 전역 타임라인) */
  if (window.gsap) {
    try {
      gsap.globalTimeline.pause();
      document.querySelectorAll('.cs-scene').forEach(function (s, i) {
        gsap.set(s, { autoAlpha: i === 0 ? 1 : 0 });
      });
    } catch (e) { /* 멈추지 못해도 편집 자체는 된다 */ }
  }
  ...
  css.textContent = [
    ...
    '[data-edit]:hover{outline-color:rgba(255,77,0,.55)}',
    '[data-edit]:focus{outline:2px solid #FF4D00;background:rgba(255,77,0,.07)}',
    '[data-edit].is-dirty{background:rgba(255,77,0,.13);outline-color:#FF4D00}',
    ...
    '#edt .st b{color:#FF4D00;font-weight:800}',
    ...
    '#edt button.pri{background:#FF4D00;color:#191919;font-weight:800}',
  ...
  /* 이 버전의 포트폴리오는 웹 전용 — PDF 는 이력서 페이지에서만 의미가 있다 */
  var isResume = /이력서/.test(decodeURIComponent(location.pathname));
  if (!isResume) bar.querySelector('#edt-pdf').remove();

  var elStatus = bar.querySelector('.st');
  var btnSave = bar.querySelector('#edt-save');
  var btnReset = bar.querySelector('#edt-reset');
  var btnPdf = bar.querySelector('#edt-pdf');   /* 포트폴리오에서는 null */
  var busy = false;
